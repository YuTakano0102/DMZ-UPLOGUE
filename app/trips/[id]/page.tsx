...
function formatDateRange(start: string, end: string) {
  ...
  const startStr = s.toLocaleDateString("ja-JP", {
    month: "short",
    day: "numeric",
  })
  if (start === end) return startStr
  const endStr = e.toLocaleDateString("ja-JP", {
    month: "short",
    day: "numeric",
  })
    ...
    <div className="flex min-h-dvh flex-col bg-background">
      ...
      <div className="sticky top-0 z-30 border-b border-border bg-card/95 backdrop-blur-lg">
        <div className="flex px-4">
          {[
            { key: "timeline" as const, icon: List, label: "タイムライン" },
            { key: "map" as const, icon: Map, label: "地図" },
          ].map((tab) => (
            <button
              key={tab.key}
